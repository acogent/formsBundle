...
class FormsCRUDController extends Controller
{
    ...
    private function getFormatJson($em, $bundle, $table, $filters, $params)
    {
        ...
        if ($search == 'true' && $searchField == 'false') {
            ...
            $builder = $em->getRepository($entity)->createQueryBuilder('a')->select('a');
        ...
        elseif ($search == 'true' && $searchField != 'false') {
...
// Si on valide le formulaire par Ajax, la redirection se fait en JQuery.
        ...
        } elseif ($ajax == 'validate' && !$form->isValid()) {
...
if (! $obj->{'get'.ucfirst($value)}()) {
                    $fields[$value] = '';
} else {
    $fields[$value] = $obj->{'get'.ucfirst($value)}()->format('Y-m-d');
}
            } else {
    ...
    public function deleteAction($bundle, $table, $id, Request $request)
    {
        ...
        if ($form->isValid()) {
            $em     = $this->getDoctrine()->getManager($this->container->getParameter('sgn_forms.orm'));
            $entity = $em->getRepository($bundle.':'.$table)->findOneById($id);
...
for ($i = 0; $i < $limit; $i++) {
                ...
                if (method_exists($enr, $method_name)) {
        $datas = $enr->$method_name();
                }
